#!/usr/bin/env python3
"""
重新提取 EP 類專利的 Claims — 使用獨立 .py 檔避免 f-string dict 問題
"""
import json
import sys
import os
...
# 提取單篇專利的腳本路徑
EXTRACTOR_SCRIPT = os.path.join(SKILL_DIR, 'scripts', '_extract_single_patent.py')
...
def main():
    ...
    for pid in EP_PATENTS:
        ...
        print(f"\nRe-extracting {pid}...", flush=True)
        
        try:
            result = subprocess.run(
                [sys.executable, EXTRACTOR_SCRIPT, url, pid],
            ...
            desc_len = len(new_data.get('description', ''))
            abs_len = len(new_data.get('abstract', ''))
            print(f"  {pid}: desc={desc_len} c1={c1_len} c2={c2_len} abs={abs_len}", flush=True)
            ...
            updated = []
            for key in ['claim_1', 'claim_2', 'claim_3', 'abstract', 'description']:
                new_val = new_data.get(key, '')
                if isinstance(new_val, str) and len(new_val) > len(existing.get(key, '')):
                    existing[key] = new_val
                    updated.append(f"{key}={len(new_val)}")
            
            if new_data.get('title') and not existing.get('title'):
                existing['title'] = new_data['title']
            if new_data.get('patent_number') and not existing.get('patent_number'):
                existing['patent_number'] = new_data['patent_number']
    ...
    print(f"\n{'='*60}")
    print("EP patent re-extraction complete")
