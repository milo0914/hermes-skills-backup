...
def check_environment():
    ...
    if torch.cuda.is_available():
        ...
        if cc[0] >= 5:  # v3.5: 放寬 GPU 限制 (sm_50+ 即可跑 Transformer)
            gpu_compatible = True
        ...
        for stock_id, group in df.groupby("stock_id"):
            ...
            if us_dfs:
                for feat_name, us_df in us_dfs.items():
                    ...
                    g[feat_name] = g[feat_name].ffill().fillna(0)
                # 確保三個因子都存在
...
class GRPOAlphaTrainer:
    ...
    def init_torch(self):
        ...
        self.scaler = torch.amp.GradScaler('cuda', enabled=False)  # v3.5: disable scaler (REINFORCE doesn't need mixed precision)
    ...
    def train_torch_regime(self, feat_tensor: np.ndarray, returns: np.ndarray,
        ...
        4. REINFORCE Policy Gradient: L = -mean(log π(a|s) * A(s,a))  [v3.5 修復零收斂]
        5. 過擬合懲罰嵌入 reward
    ...
    # --- v3.5: REINFORCE Policy Gradient (修復零收斂) ---
    # PPO clipped surrogate 在 on-policy GRPO 中 loss=0 (ratio≡1, advantages 標準化均值=0)
    # 改用 REINFORCE: L = -mean(log_prob * advantage)，這才是 GRPO 原論文正確實現
    ...
    # 計算 ratio 僅供監控（不參與梯度）
    with torch.no_grad():
        ratio = torch.ones_like(log_probs_tensor)  # REINFORCE 不需要 ratio
        if hasattr(self, '_old_log_probs') and self._old_log_probs is not None:
            old_lp = torch.tensor(self._old_log_probs, dtype=torch.float32,
                                  device=self.config.device)
            if len(old_lp) == len(log_probs_tensor):
            ...
            # v3.5: REINFORCE 直接 backward（不需要 GradScaler）
            loss.backward()
            ...
            if has_nan:
                ...
                self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.config.lr,
                    weight_decay=1e-5)
                # v3.5: no scaler needed for REINFORCE
                self._old_log_probs = None
                continue

            # v3.5: 儲存 old_log_probs 僅供監控 ratio (REINFORCE 不需要 importance sampling)
...
def main():
    ...
    # --- 步驟 1: 載入/生成數據 ---
    print("\n--- 步驟 1: 數據準備 ---")
    data_path = "/kaggle/input/twstock-grpo-training-data"

    if os.path.exists(data_path):
        print(f"  從 Kaggle Dataset 載入: {data_path}")
        ...
        if os.path.exists(foi_path):
            futures_oi_df = pd.read_csv(foi_path)
            if "date" in futures_oi_df.columns:
                futures_oi_df["date"] = pd.to_datetime(futures_oi_df["date"])

        # 美股指數
        us_indices_df = None
        us_path = os.path.join(data_path, "us_indices.csv")
        if os.path.exists(us_path):
            us_indices_df = pd.read_csv(us_path)
            if "date" in us_indices_df.columns:
                us_indices_df["date"] = pd.to_datetime(us_indices_df["date"])
    else:
    ...
    for stock_id, res in results.items():
        h = []
        if "history" in trainer.history:
            for entry in trainer.history:
                h.append({k: float(v) if isinstance(v, (np.integer, np.floating)) else v
                          for k, v in entry.items()})
