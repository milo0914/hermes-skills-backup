...
### Critical Discovery (2026-05-19): Report File Not Found
**Problem**: User asked "Where is the Merck patent search report .md file from session 4/29~5/12?"

**Investigation**:
- Searched `/tmp/*.md`, `/data/.hermes/sessions/*.json`, `/home/appuser/*.md`
- Found skill reference files (`patent-database-requirements.md`, `patent-search-strategy.md`) 
- **No standalone patent search report was found** at any predictable path

**Root Cause**: The patent search task from 4/29~5/12 session completed with findings embedded in session history, but **no standalone .md report file was generated** at a known location like `/tmp/merck-patent-report.md`.

**Lesson Learned**: Patent search tasks MUST conclude with:
1. **Explicit report generation**: `cat > /tmp/merck-patent-report.md << 'EOF'...`
2. **Clear file path communication**: Tell user "Report saved to `/tmp/merck-patent-report.md`"
3. **Session-independent storage**: Don't rely on session history for critical deliverables

**Best Practice**: At task start, announce: "I will save the patent search report to `/tmp/merck-patent-report.md` when complete."
