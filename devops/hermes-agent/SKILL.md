...
- Discover or install new skills

## Absorbed Skills

### find-skills (archived)
The `find-skills` skill helped users discover and install agent skills from the open skills ecosystem. Its functionality — searching skill registries, installing from clawhub/skills.sh, and browsing available capabilities — is now a sub-domain of this umbrella.

**Details:** `references/find-skills.md`, `references/find-skills_hermes-installation-patterns.md`
...
### Terminal Tool Environment Variable Expansion

When reading file content via `terminal(command='cat file.py')`, the tool's output display may expand environment variable names that appear in the file content. For example, if a Python file contains `os.environ.get("LINE_CHANNEL_ACCESS_TOKEN", "")`, the terminal output may replace `LINE_CHANNEL_ACCESS_TOKEN` with its actual value, corrupting the string if you then use it for Python `str.replace()`.

**Safe patterns for file content modification:**
1. Use `write_file` to write a helper Python script (`/tmp/fix.py`) that opens the target file with `open(path, 'rb')`, performs replacement on raw bytes/str, and writes back. Then run `python3 /tmp/fix.py`.
2. Use `open(path, 'r').read()` inside `execute_code` rather than `terminal('cat ...')` to get uncorrupted content.
3. For verification, check raw bytes rather than trusting terminal output of file content.
...
**Session discoveries (2026-06-06):**
- `gstack` (garrytan/gstack) - DANGEROUS verdict (CRITICAL persistence: modifies CLAUDE.md; HIGH privilege_escalation: allowed-tools; MEDIUM supply_chain: git clone). All 8 skills require --force. Installs as `gstack-upgrade` (frontmatter name differs from search name). Sub-skills: office-hours, plan-ceo-review, plan-design-review, plan-eng-review, qa, review, autoplan. Use `skills-sh/garrytan/gstack/<name>` full path to avoid wrong-author matches. See `references/skill-sources.md` §6 for batch install loop.
