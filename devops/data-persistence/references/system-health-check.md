...
### 2b. Check Web UI Database Status
If the Web UI is deployed, also check its separate database:
```bash
# Check Web UI DB exists
test -f ~/.hermes-web-ui/hermes-web-ui.db && echo "Web UI DB OK" || echo "Web UI DB MISSING"

# Count Web UI sessions and messages
python3 -c "import sqlite3, os; conn = sqlite3.connect(os.path.expanduser('~/.hermes-web-ui/hermes-web-ui.db')); print('Web UI Sessions:', conn.execute('SELECT COUNT(*) FROM sessions').fetchone()[0]); print('Web UI Messages:', conn.execute('SELECT COUNT(*) FROM messages').fetchone()[0])"
```

If CLI session count >> Web UI session count, the sync cron job may need attention. See skill `hermes-cli-to-webui-sync`.
...
- Web UI Database: `~/.hermes-web-ui/hermes-web-ui.db` (separate from CLI DB)
...
- `hermes-cli-to-webui-sync`: For syncing CLI sessions to Web UI DB
