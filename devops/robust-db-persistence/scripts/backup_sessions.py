...
# ============================================================
# Secret Variable Resolution (HF Spaces / Docker)
# ============================================================

def read_proc1_environ():
    """Read environment variables from PID 1's /proc/1/environ.
    
    HF Spaces injects secrets (HF_TOKEN, HF_DATASET_REPO, etc.) into 
    the main process environment, but child processes (cron scripts) 
    may not inherit them via os.environ. This function reads them 
    directly from /proc/1/environ as a fallback.
    """
    env_vars = {}
    try:
...
def get_hf_secret(key, default=""):
    ...
    Resolution order:
    1. os.environ (standard env vars)
    2. /proc/1/environ (HF Spaces secret variables)
    3. /data/.hermes/.env file (manual config)
    4. default value
    """
    # Source 1: os.environ
    ...
    # Source 2: /proc/1/environ (HF Spaces secrets)
