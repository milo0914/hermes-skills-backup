#!/usr/bin/env python3
"""
Self-healing cron wrapper for session restore from HuggingFace.
Problem: /data/.hermes/scripts/ is ephemeral (wiped on HF Space rebuild).
Solution: This wrapper auto-heals before running the actual restore.
...
REAL_SCRIPT = os.path.join(SKILLS_DIR, "devops/robust-db-persistence/scripts/restore-from-hf.py")
...
def main():
    ...
    if os.path.isfile(REAL_SCRIPT):
        ...
        print(f"ERROR: Script still not found after self-heal")
        sys.exit(1)
